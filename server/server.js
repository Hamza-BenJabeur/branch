...
app.post('/pushTodo', (req , res)=>{
  ...
  Name.find({} , function(err , result){
    ...
    User.updateOne({name :result[0].hashem } , {$push:{myToDoList : req.body.todo}} , function(err , result){
      if(err) console.log(err) 
    })
  })
})
...
app.post('/SEND' , (req,res)=>{
  ...
  User.updateOne({name : req.body.friend}, {$push:{myToDoList : iSend}} , function(err , result){
    if(err) console.log(err) 
  })
})
